#### from /usr/local/lib/python3.10/dist-packages/lightning/pytorch/plugins/precision/precision_plugin.py
...
class PrecisionPlugin(FabricPrecision, CheckpointHooks):
    ...
    def optimizer_step(  # type: ignore[override]
        ...
        **kwargs: Any,
        ...
        ret = optimizer.step(closure=closure, **kwargs)
        #return optimizer.step(closure=closure, **kwargs)
        ### NEW CODE
        for p in model.parameters():
            if p.grad is not None:
                del p.grad
        model.model.diffusion_model.to("cuda:0")
        model.cond_stage_model.to("cuda:0")
        model.first_stage_model.to("cuda:0")
        ### END NEW CODE
        return ret
